...
export default function InvestmentRequestsPage() {
  ...
  const getProfileStyles = (profile: string) => {
    switch (profile) {
      ...
      case 'SILVER':
        return { bg: 'bg-slate-400/10', text: 'text-slate-400', border: 'border-slate-400/20' };
      case 'GOLD':
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400">Loading investment requests...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-50 selection:bg-purple-500/30">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-400">Review and approve pending investment requests from your referrals</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          {['BRONZE', 'SILVER', 'GOLD'].map((profile) => {
            ...
            return (
              <div key={profile} className="bg-slate-900/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-800">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-slate-400 text-sm font-medium">{profile}</span>
        ...
        {/* Profile Filter */}
        <div className="bg-slate-900/50 backdrop-blur-xl rounded-2xl border border-slate-800 p-4 mb-8">
          <div className="flex items-center gap-2 flex-wrap">
            <Filter className="w-5 h-5 text-slate-500" />
            <span className="text-sm font-medium text-slate-400 mr-2">Filter by Profile:</span>
        ...
        {investments.length > 0 ? (
          <div className="space-y-4">
            {investments.map((investment) => {
              ...
              return (
                <div
                  key={investment.id}
                  className="group bg-slate-900/50 backdrop-blur-xl rounded-2xl border border-slate-800 p-6 hover:border-purple-500/50 transition-all"
                >
                  <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                    <div className="flex-1 w-full">
                      ...
                      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 bg-slate-950/50 rounded-xl p-4 border border-slate-800">
                        {investment.user && (
                          <div>
                      ...
                      {investment.notes && (
                        <div className="mt-4 p-3 bg-slate-950/30 rounded-lg border border-slate-800/50">
                          <p className="text-xs text-slate-500 mb-1">Notes:</p>
                          <p className="text-sm text-slate-300 italic">"{investment.notes}"</p>
                        </div>
                      )}
                    </div>
          ...
          <div className="bg-slate-900/50 backdrop-blur-xl rounded-3xl border border-slate-800 p-12 text-center">
            ...
            <p className="text-slate-400 font-medium text-lg mb-2">No pending investment requests</p>
      ...
      {showApproveModal && selectedInvestment && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="text-xl font-bold text-white mb-4">Approve Investment</h3>
            <p className="text-slate-400 mb-6">
              Are you sure you want to approve this <span className="text-white font-bold">{selectedInvestment.amount} USDT</span> {selectedInvestment.profile} investment?
            </p>
            <div className="mb-6">
      ...
      {showRejectModal && selectedInvestment && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="text-xl font-bold text-white mb-4">Reject Investment</h3>
            <p className="text-slate-400 mb-6">
              Please provide a reason for rejecting this <span className="text-white font-bold">{selectedInvestment.amount} USDT</span> investment.
            </p>
            <div className="mb-6">
              <label className="block text-sm font-medium text-slate-300 mb-2">
                Rejection Reason *
