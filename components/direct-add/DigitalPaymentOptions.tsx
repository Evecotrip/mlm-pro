      ...
      <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-5">
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <span className="font-bold text-white text-lg">{parseFloat(amount).toLocaleString('en-IN')}</span>
          </div>
        ...
        <div className="space-y-4">
          ...
          >
            <div className="flex items-center gap-4 relative z-10">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${selectedOption === 'qr-code' ? 'bg-blue-500 text-white' : 'bg-slate-800 text-slate-400 group-hover:text-blue-400'
                }`}>
                <QrCode className="w-6 h-6" />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <p className={`font-bold text-lg ${selectedOption === 'qr-code' ? 'text-white' : 'text-slate-200'}`}>
                    Request for QR Code
                  </p>
                  {selectedOption === 'qr-code' && <Check className="w-5 h-5 text-blue-500" />}
                </div>
                <p className="text-sm text-slate-400">Pay via UPI using QR code</p>
              </div>
            </div>
          </button>
          ...
          >
            <div className="flex items-center gap-4 relative z-10">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${selectedOption === 'bank-details' ? 'bg-emerald-500 text-white' : 'bg-slate-800 text-slate-400 group-hover:text-emerald-400'
                }`}>
                <Building2 className="w-6 h-6" />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <p className={`font-bold text-lg ${selectedOption === 'bank-details' ? 'text-white' : 'text-slate-200'}`}>
                    Request for Bank Details
                  </p>
                  {selectedOption === 'bank-details' && <Check className="w-5 h-5 text-emerald-500" />}
                </div>
                <p className="text-sm text-slate-400">Pay via NEFT/RTGS/IMPS</p>
              </div>
            </div>
          </button>
        </div>
