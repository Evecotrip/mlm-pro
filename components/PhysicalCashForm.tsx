...
export default function PhysicalCashForm({ onClose, onBack, currentUser, withdrawalInfo }: PhysicalCashFormProps) {
          ...
          <div className="p-8 text-center">
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 mb-6 space-y-4 text-left">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-white">Physical Cash Pickup</span>
              </div>
              <div className="flex justify-between items-center">
  ...
  if (step === 'contact') {
    if (!referrer) {
      return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-slate-950 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-lg p-8 text-center">
            <AlertCircle className="w-16 h-16 text-red-500 mx-auto mb-4" />
            <h2 className="text-xl font-bold text-white mb-2">Referrer Not Found</h2>
            <p className="text-slate-400 mb-6">Unable to find your referrer's contact information</p>
            <button
              onClick={onBack}
        ...
        <div className="bg-slate-950 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="sticky top-0 bg-slate-950/80 backdrop-blur-xl border-b border-slate-800 p-6 z-10 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Physical Cash Pickup</h2>
                <p className="text-slate-400 text-sm">Contact your referrer</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-blue-300 mb-1">Contact Your Referrer</p>
            ...
            {/* Withdrawal Amount */}
            <div className="bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-xl p-5 text-center">
              <p className="text-sm text-emerald-400 font-medium mb-2">Withdrawal Amount</p>
              <p className="text-4xl font-bold text-emerald-300">₹{parseFloat(amount).toLocaleString('en-IN')}</p>
            </div>
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5">
              <div className="flex items-center gap-4 mb-5">
                ...
                <div className="flex-1">
                  <p className="text-sm text-slate-400 mb-1">Your Referrer</p>
                  <h3 className="text-xl font-bold text-white">{referrer.name}</h3>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-slate-950 border border-slate-800 rounded-xl p-4">
                  <p className="text-xs text-slate-500 mb-2 uppercase tracking-wider">Primary Contact</p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Phone className="w-4 h-4 text-slate-400" />
                      <span className="font-mono font-semibold text-slate-200">{referrer.phone}</span>
                    </div>
                    <a
            ...
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
              <p className="text-sm text-amber-200/80">
                <strong className="text-amber-400">Note:</strong> Your referrer has been notified via SMS, email, and in-app notification about your withdrawal request.
              </p>
            </div>
      ...
      <div className="bg-slate-950 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <div className="sticky top-0 bg-slate-950/80 backdrop-blur-xl border-b border-slate-800 p-6 z-10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Physical Cash Pickup</h2>
              <p className="text-slate-400 text-sm">Enter withdrawal amount</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleAmountSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 flex justify-between items-center">
            <div>
              <p className="text-sm text-emerald-400 mb-1">Available for Withdrawal</p>
              <p className="text-2xl font-bold text-emerald-300">₹{withdrawalInfo.availableBalance.toLocaleString('en-IN')}</p>
            </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="0"
                className="w-full pl-10 pr-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all text-xl font-bold placeholder:font-normal placeholder:text-slate-600"
                min="1"
          ...
          <div className="grid grid-cols-4 gap-3">
            {[500, 1000, 2000, 5000].map((quickAmount) => (
              <button
                ...
                className="py-2 px-3 bg-slate-900 text-slate-300 border border-slate-800 hover:border-emerald-500/50 hover:text-emerald-400 rounded-lg text-sm font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                ₹{quickAmount}
              </button>
            ))}
