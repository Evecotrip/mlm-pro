...
export default function InvestmentModal({ onClose, currentUser, onSuccess }: InvestmentModalProps) {
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200 relative">
        {/* Background Effects */}
        ...
        <div className="sticky top-0 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 p-6 z-10 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">New Investment</h2>
            <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">Create a new investment request</p>
          </div>
          <button
            onClick={onClose}
