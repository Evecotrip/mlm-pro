...
import { LogOut, UserCheck, Wallet, HandCoins, Bell, ChevronDown, Clock, DollarSign, Banknote } from 'lucide-react';
          ...
          <div className="flex items-center gap-3">
            ...
            {showWalletButton && (
              <button
                ...
                className="group relative flex items-center gap-3 pl-4 pr-5 py-2 bg-slate-900 border border-slate-800 hover:border-emerald-500/50 rounded-full transition-all duration-300 hover:shadow-[0_0_20px_-5px_rgba(16,185,129,0.3)]"
              >
                <div className="flex flex-col items-end leading-none">
                  <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">Balance</span>
                  <span className="text-sm font-bold text-white group-hover:text-emerald-400 transition-colors">
                    {walletBalance.toLocaleString('en-IN')} USDT
                  </span>
                </div>
            ...
            <div className="flex items-center gap-2 bg-slate-900/50 border border-slate-800 rounded-full p-1">
              {/* User Requests */}
            ...
            <div className="flex items-center gap-3 pl-3 border-l border-slate-800">
              {!isLoading && userName && (
                <div className="hidden md:block text-right">
                  ...
                  <p className="text-[10px] text-slate-500 uppercase tracking-wider">Verified User</p>
                </div>
              )}

              <button
                onClick={onLogout}
                className="group p-2 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded-full transition-all"
                title="Logout"
              >
