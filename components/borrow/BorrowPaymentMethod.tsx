...
export default function BorrowPaymentMethod({ lenderName, amount, onContinue, onBack }: BorrowPaymentMethodProps) {
  ...
  return (
    <div className="space-y-8">
      {/* Summary */}
      <div className="bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 rounded-xl p-5 flex justify-between items-center">
        <div>
          <p className="text-xs text-slate-500 dark:text-slate-500 uppercase tracking-wider mb-1">Borrowing from</p>
          <p className="font-bold text-slate-900 dark:text-white text-lg">{lenderName}</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-slate-500 dark:text-slate-500 uppercase tracking-wider mb-1">Amount</p>
          <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">₹{parseFloat(amount).toLocaleString('en-IN')}</p>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          >
            <div className="flex items-center gap-4 relative z-10">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${paymentMethod === 'soft-cash' ? 'bg-purple-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 group-hover:text-purple-600 dark:group-hover:text-purple-400'
                }`}>
                <CreditCard className="w-6 h-6" />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <p className={`font-bold text-lg ${paymentMethod === 'soft-cash' ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-200'}`}>
                    Soft Cash (Digital)
                  </p>
                  {paymentMethod === 'soft-cash' && <Check className="w-5 h-5 text-purple-500" />}
                </div>
                <p className="text-sm text-slate-500 dark:text-slate-400">Receive money via bank transfer or UPI</p>
              </div>
            </div>
          </button>
          ...
          >
            <div className="flex items-center gap-4 relative z-10">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${paymentMethod === 'hard-cash' ? 'bg-emerald-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400'
                }`}>
                <Banknote className="w-6 h-6" />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <p className={`font-bold text-lg ${paymentMethod === 'hard-cash' ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-200'}`}>
                    Hard Cash (Physical)
                  </p>
                  {paymentMethod === 'hard-cash' && <Check className="w-5 h-5 text-emerald-500" />}
                </div>
                <p className="text-sm text-slate-500 dark:text-slate-400">Collect cash in person from the lender</p>
              </div>
            </div>
          </button>
        </div>
